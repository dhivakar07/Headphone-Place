...
function Headphones() {
  ...
  const handleproduct = (item) => {
    navigate(`/product/Headphones/${item._id}`);
  };
  ...
  const handleProductRemove = async (id) => {
    try {
      const res = await axios.delete(
        `http://localhost:3000/product/Headphones/remove/${id}`,
        {
          id,
        },
      );
    ...
    <>
      ...
      <section className="product_section">
        <div className="product_banner">
          <img src="/headphone-banner.jpg" alt="" />
          <div className="product_banner-header">
            <h1>Wired Headphones</h1>
            <p>
              Browse the ultimate range of Wired Headphones, curated for every
              audiophile
            </p>
          </div>
        </div>
