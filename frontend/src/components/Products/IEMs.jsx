...
function IEMs() {
  ...
  useEffect(() => {
    const getproduct = async () => {
      try {
        const res = await axios.get("http://localhost:3000/products/IEMs");
  ...
  const handleproduct = (item) => {
    navigate(`/product/IEMs/${item._id}`);
  };
  ...
  const handleProductRemove = async (id) => {
    try {
      const res = await axios.delete(
        `http://localhost:3000/product/IEMs/remove/${id}`,
        {
          id,
        },
      );
    ...
    <>
      ...
      <section className="product_section">
        <div className="product_banner">
          <img src="/iembanner.jpg" alt="" />
          <div className="product_banner-header">
            <h1>In-Ear Monitors</h1>
            <p>
              Browse the ultimate range of In-Ear Monitors, curated for every
              audiophile
            </p>
          </div>
        </div>
